...
def extract_one_video(folder_name, video_name, args):
    if not video_name.endswith(".avi"):
            return
    
    save_path = os.path.join(
            args.jpg_dir, folder_name, video_name[:-4])
    video_path = os.path.join(args.raw_video_dir, folder_name, video_name)
...
def extract_one_folder(folder_name, args):
    folder_path = os.path.join(args.raw_video_dir, folder_name)
    for video_name in os.listdir(folder_path):
        extract_one_video(folder_name, video_name, args)
...
def main():
    ...
    os.chdir(args.raw_video_dir)
    folder_names = [d for d in os.listdir('.') if os.path.isdir(d)]

    _func = functools.partial(extract_one_folder, args=args)
    p = Pool(NUM_THREADS)
    r = list(tqdm(
        p.imap(_func, folder_names),
