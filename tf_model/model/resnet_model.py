...
class Model(object):
  ...
  def __call__(
      self, inputs, training, get_all_layers=None, skip_final_dense=False, final_pooling=None,
    ...
    with self._model_variable_scope():
      ...
      if self.pre_activation:
        ...
        if slowfast_single_pooling:
          ...
          inputs = tf.reshape(inputs, [bs, 1 * 1 * self.final_size])
      ENDING_POINTS.append(inputs)
      ...
      if skip_final_dense:
        ...
        if final_pooling is None:
          return tf.reshape(inputs, [bs, 7 * 7 * self.final_size])
        if final_pooling == 1:
          axes = [2, 3] if self.data_format == 'channels_first' else [1, 2]
          inputs = tf.reduce_mean(inputs, axes, keepdims=True)
...
class TRNModel(Model):
  def __init__(self, resnet_size, data_format=None, num_classes=_NUM_CLASSES,
               resnet_version=DEFAULT_VERSION,
               dtype=DEFAULT_DTYPE, use_mean=False):
    """
    Args:
      resnet_size: The number of convolutional layers needed in the model.
      data_format: Either 'channels_first' or 'channels_last', specifying which
  ...
  def _preprocess_data(self, inputs):
    ...
    self.trn_num_frames = curr_shape[1]
    inputs = tf.reshape(inputs, [-1] + curr_shape[2:])
    return inputs
  
  def _get_final_dense(self, inputs):
    inputs = tf.reshape(inputs, [-1, self.trn_num_frames, self.final_size])
    multi_scale_rel = trn_model.MultiScaleRelation(
        self.trn_num_frames, 512, use_mean=self.use_mean)
    inputs = multi_scale_rel(inputs)
...
class TRNTileModel(Model):
  def __init__(self, resnet_size, data_format=None, num_classes=_NUM_CLASSES,
               resnet_version=DEFAULT_VERSION,
               dtype=DEFAULT_DTYPE, use_mean=False, trn_num_frames=8):
    """
    Args:
      resnet_size: The number of convolutional layers needed in the model.
      data_format: Either 'channels_first' or 'channels_last', specifying which
    ...
    self.use_mean = use_mean
    super(TRNTileModel, self).__init__(
        resnet_size=resnet_size,
        bottleneck=bottleneck,
        num_classes=num_classes,
        num_filters=64,
  ...
  def _get_final_dense(self, inputs):
    ...
    inputs = tf.tile(inputs, [1, self.trn_num_frames, 1])
    multi_scale_rel = trn_model.MultiScaleRelation(
        self.trn_num_frames, 512, use_mean=self.use_mean)
    inputs = multi_scale_rel(inputs)
