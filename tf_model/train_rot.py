...
import train_vie
ROT_KINETICS_VIDEOS = 239871
ROT_KINETICS_VAL_VDS = 19647
...
def get_params_from_arg(args):
    ...
    # train_params
    train_data_loader = rot_data.get_train_rot_pt_loader(args)
    ...
    def train_loop(sess, train_targets, num_minibatches=1, **params):
        ...
        data_en_update_fre = ROT_KINETICS_VIDEOS // args.batch_size
        if global_step % data_en_update_fre == 0:
            data_enumerator.pop()
            data_enumerator.append(enumerate(train_data_loader))
        _, image = next(data_enumerator[0])
        feed_dict = rot_data.get_feeddict(image)
        sess_res = sess.run(train_targets, feed_dict=feed_dict)
        return sess_res

    train_data_param = {
            'func': rot_data.get_rot_placeholders,
    ...
    # validation_params
    val_len = 3 * ROT_KINETICS_VAL_VDS
    topn_val_data_param = {
            'func': rot_data.get_rot_placeholders,
    ...
    val_data_loader = rot_data.get_val_rot_pt_loader(args)
    val_counter = [0]
    val_data_enumerator = [enumerate(val_data_loader)]
    def valid_loop(sess, target):
        val_counter[0] += 1
        if val_counter[0] % (ROT_KINETICS_VAL_VDS // args.test_batch_size) == 0:
            val_data_enumerator.pop()
            val_data_enumerator.append(enumerate(val_data_loader))
        _, image = next(val_data_enumerator[0])
        feed_dict = rot_data.get_feeddict(image, name_prefix='VAL')
    ...
    # model_params
    model_params = {
            'func': rot_model.build_loss_accuracy,
...
def get_config():
    ...
    cfg.add('rot_real_prep', type=bool,
            help='Train preprocessing being real resize')
    return cfg
